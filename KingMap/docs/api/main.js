var pp = window.location.href.indexOf("#api-");
if(pp!=-1){
	var url = window.location.href.substring(0,pp);
...
require([
	...
	var templateHandler = KRTemplate[apiProject.templateType];
	if(templateHandler!=null){
		templateHandler.before(apiProject, apiData);
	}
...
*/
                ...
                reversed.forEach(function (item, index) {
                    ...
                    if (parts.length - 1 == dot_count) {
                        ...
                        if (parts.slice(1).length  >= 1) {
                            ...
                            if (prefix_index > -1) {
								var n1 = fields_names.indexOf(item.field);
                                fields.splice(n1, 1);
                                fields.splice(n1<prefix_index?prefix_index:prefix_index + 1, 0, item);
                            }
                        }
                    }
                });
	...
	var mainUL = $("<ul class=\"navMenu nav nav-list list\">");
	var createGroups = function(ul,name,value,level){
		value = value==null?name:value;
		level = level==null?0:level;
		var poit  = name.indexOf(".");
		if(poit==-1){
			li = createGroup(ul,name,value,level);
			
			
			return li;
		}else{
			var n1 = name.substring(0,poit);
			var n2 = name.substring(poit+1);
			var li = createGroup(ul,n1,value,level);
			ul.append(li);
			var subUL = li.find("UL:first");
			return createGroups(subUL,n2,value,level+1);
		}
	}
	var createGroup = function(ul,name,value,level){
		var li = ul.find("li[value='"+name+"']");
	...
	$.each(nav,function(i,item){
		...
		if(item.group!='undefined')
			 li = createGroups(mainUL,item.group);
		if(item.name!=null){
			var ul = li==null?mainUL:li.find("UL:first");
			var key = item.group+"-"+item.name;
			var item2 = ul.find("li[value='"+key+"']");
			if(item2.length>0){return;}
			item2 = $("<li><a href=\"#api-"+key+"\" ><span>"+item.title+"</span></a></li>");
			item2.attr("value",key);
			item2.addClass("nav-item");
			ul.append(item2);
		}
	});
	mainUL.attr("id","mainUL");
	$("#slimtest1").append(mainUL);
	var searchUL = $("<ul class=\"navMenu nav nav-list list\">");
	searchUL.attr("id","searchUL");
	searchUL.hide();
	$("#slimtest1").append(searchUL);
	//处理搜索
	var searchDIV = $(".sidenav-search");
    $(document).keyup(function(e) {
      if (e.keyCode === 27){ searchDIV.find(".search-reset").click();}
    });
	searchDIV.find(".search-reset").click(function(){
		searchDIV.find("input").val('').focus();
		mainUL.show();
		searchUL.hide();
		searchUL.find("li").detach();
	});
	var timer = null;
	searchDIV.find("input").on('input',function(e){  
			var input = $(this);
			if(timer!=null){
			...
			timer = window.setTimeout(function(){
				searchUL.find("li").detach();
				//搜索数据，只搜索内容，不搜索分组
				var lis = mainUL.find(".nav-item:contains("+input.val()+")");
				$.each(lis,function(i,item){
					item = $(item);
					searchUL.append(item.clone());
				});
				mainUL.hide();
				searchUL.show();
			},500);
	...
	$("h1:contains(undefined)").detach();
	if(templateHandler!=null){
	templateHandler.after();
	}
});
}