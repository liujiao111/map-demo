...
(function(){
	KingMap.Map = function(options){
		...
		* 高德使用LngLat 类
		*/
		var setCenter = function(kpPoint){
			map.setCenter(kpPoint.getPoint(ops.apiType));
		};	
		...
		* 高德使用LngLat 类
		*/
		var centerAndZoom = function(kpPoint,zoom){
			if(ops.apiType=="BMap"){
				map.centerAndZoom(kpPoint.getPoint(ops.apiType),zoom);
			}else{
				map.setZoomAndCenter(zoom,kpPoint.getPoint(ops.apiType));
			}
		};
		...
		*/
		...
		var addEventListener = function(event,handler){
			...
			var methodMap = {
				"AMap" : function(){
					map.on(event,function({type,target,pixel,lnglat}){
				...
				"BMap" : function(){
					map.addEventListener(event,function({type,target,point,pixel}){
						MapsEvent = Object.assign(MapsEvent,{
							type : type,
							target : target,
